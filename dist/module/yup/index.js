            ...
            leave: (node) => {
                ...
                if (this.config.enumsAsTypes) {
                    ...
                    const values = node.values
                        ?.map(enumOption => `${enumname}.${visitor.convertName(enumOption.name, {
...
const generateFieldYupSchema = (config, visitor, field, indentCount) => {
    const generatedCodesForDirectives = buildApi(field.name.value, config.rules ?? {}, config.ignoreRules ?? [], field.directives ?? []);
...
const generateFieldTypeYupSchema = (config, visitor, type, parentType, generatedCodesForDirectives) => {
    if (isListType(type)) {
        ...
        return `yup.array(${maybeLazy(config, type.type, `${gen}.defined()`)})${generatedCodesForDirectives.rulesForArray}${nullable ? '.nullable()' : '.defined()'}`;
    }
